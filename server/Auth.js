...
class Auth {
     ...
     */
    static findUid(uid, success, failure, reqs) {
        function fail(error) {
     ...
     * @param {genericFailure} failure Callback to be run if no user is found.
     */
    static findQuery(query, success, failure) {
        function fail(error) {
    ...
    static register(data, success, failure) {
        ...
        new Chain(function() { // Try to find preexisting user
            Auth.find(newUser.username, newUser.email,
                () => { // User found
                    errors.push({
                        "errorName": "taken",
    ...
    static login(username, password, success, failure) {
        ...
        Auth.find(username, undefined, // Leave email undefined
            user => { // User found
                Bcrypt.compare(
     ...
     * @param {genericFailure} failure Callback to be run in case of error.
     */
    static edit(ctx, success, failure) {
        function fail(error) {
        ...
        let [user, edit] = [ctx.user, ctx.edit];
        ...
        new Chain(function() {
            ...
            if (edit.email && (edit.email != user.email)) {
                user.verified = false;
                Auth.find(undefined, edit.email,
                    () => { // Email taken
                        fail({
            ...
            if (edit.username && (edit.username != user.username)) {
                Auth.find(edit.username, undefined,
                    () => { // Username taken
                        fail({
            ...
            if (errUpdate || writeResult.result.ok != 1) return fail({
                "errorName": "write",
     ...
     */
    static editPassword(uid, password, success, failure) {
        function fail(error) {
     ...
     * @param {genericFailure} failure
     */
    static genApiKey(uid, success, failure) {
        function fail(error) {
     ...
     * @param {genericFailure} failure
     */
    static setVerified (uid, verified, success, failure) {
        function fail(error) {
