 ...
 */
exports.findUid = function(uid, success, failure, reqs) {
    function fail(error) {
...
exports.remove = function(uid, password, success, failure) {
    function fail(error) {
        failure(error);
    }
    ...
    new Chain(function() {
        ...
        Bcrypt.compare(
            password, user.password, 
            this.next.bind(this, user)
        );
    }, function(user, err, result) {
        // Password invalid
        if (!result) return fail({
            "errorName": "password",
            "errorNameFull": "Auth.remove.password"
        }); 
